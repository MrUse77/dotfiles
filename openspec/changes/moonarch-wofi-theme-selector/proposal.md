...
Provide a standalone MoonArch runtime selector so users can safely switch a coherent Hyprland, Waybar, Wofi, and Ghostty theme without mutating Stow-managed configuration. Retire the obsolete direct-copy `dots theme` command; `dots install` remains installer-only.
...
- Transactional installer deployment of MoonArch `bin` and `themes`; safe GNU Stow development with `--no-folding`.
...
| `scripts/stow-dev.sh`, `tests/`, `README.md` | Modified | Stow safety, tests, documentation |
...
| Stow writes into repo | Low | Require `--no-folding` |
...
- Hyprland, Waybar, Wofi, Ghostty, GNU Stow, and the existing installer transaction.
